...
function showHardCodedData() {
  ...
  let myCircle = L.circle([51.508, -0.11], 250, {
    ...
    .addTo(mymap)
    .bindPopup("I am a circle in 2024.");
  ...
  let myPolygon = L.polygon(
    ...
    .addTo(mymap)
    .bindPopup("I am a polygon in 2024.");

  // add a point
  let myMarker = L.marker([51.508, -0.1], {})
    .addTo(mymap)
    .bindPopup("this is a point<br>with two rows");
}
...
function showHardCodedGeoJSON() {
  ...
  let geoJSON = {
    ...
    properties: {
      name: "London",
      popupContent:
        "Is is where UCL is based?.  We have on campus and off campus activity.",
    },
    geometry: {
      type: "Point",
      coordinates: [37.11702, -84.061539],
    },
  };
  // and add it to the map
  let geoJSONFeature = L.geoJSON(geoJSON);
  let geoJSONLayer = geoJSONFeature
    .addTo(mymap)
    .bindPopup(
      "<b>" +
        geoJSON.properties.name +
        " " +
        geoJSON.properties.popupContent +
        "<b>"
    );
...
// <a class="dropdown-item" class="dropdown-item" href="#" onclick="showPointsWithIcons()";>Show Points With Icons</a>
...
function showPointsWithIcons() {
  ...
  let feature2 = L.geoJSON(geoJSONFeature1, {
    ...
    .bindPopup(
      "<b>" +
        geoJSONFeature1.properties.name +
        " " +
        geoJSONFeature1.properties.popupContent +
        "<b>"
    );
